...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on('destroy session', function() {
    return session.handleDisconnect(true);
  });
  ...
  socket.on('terminal options save', function(id, options) {
    return session.saveTermOptions(id, options);
  });
...
Server.prototype.getInactiveSessionById = function (userAgentId, id) {
  ...
  for (var i = 0; i < this.sessions[userAgentId].length; i++) {
    if (this.sessions[userAgentId][i].id == id
      && this.sessions[userAgentId][i].inactive) {
...
Server.prototype.removeSession = function (userAgentId, id) {
  ...
  for (var i = 0; i < this.sessions[userAgentId].length; i++) {
    if (this.sessions[userAgentId][i].id == id) {
      this.sessions[userAgentId].splice(i, 1);
    }
  }
};
...
function Session(server, socket) {
  ...
  this.user = req.user;
  this.user_agent_id = req.query.uid ? req.query.uid : (req.user || this.uid());
  ...
  if (conf.syncSession) {
    ...
    if (req.query.sessionId) {
      stale = this.server.getInactiveSessionById(this.user_agent_id, req.query.sessionId);
    }

    if (!stale) {
      stale = this.server.getInactiveSession(this.user_agent_id);
    }
...
Session.prototype.uid = function() {
  if (this.server.conf.syncSession) {
    var req = this.req;
    var uaPart = req.query.userAgentId ? req.query.userAgentId : req.headers['user-agent'];
    return req.getIp() + '|' + uaPart;
  }
  return Session.uid++ + '';
};
...
Session.prototype.sync = function() {
  ...
  this.socket.emit('sync', this.layoutState, this.termOptions);
};
...
Session.prototype.handleKill = function(id) {
  ...
  if (this.termOptions[id]) {
    delete this.termOptions[id];
  }
};
...
Session.prototype.handleDisconnect = function(forceDestroy) {
  var self = this
    , terms = this.terms
  ...
  function destroy() {
    ...
    while (i--) {
      ...
      if (termOptions[key[i]]) {
        delete termOptions[key[i]];
      }
  ...
  if (!conf.syncSession || forceDestroy === true) {
    return destroy();
  }
...
Session.prototype.saveTermOptions = function (id, options) {
  this.termOptions[id] = options;
};
