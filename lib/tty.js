...
Session.prototype.handleCreate = function(cols, rows, func, script) {
  ...
  var len = Object.keys(terms).length
    , term
    , id;
  if (this.script && len > 0){
    this.warning("cannot create another terminal, we are running scripts here");
    return;
  }
  ...
  var shell = typeof conf.shell === 'function'
    ...
    : conf.shell;
  if (this.script){
    console.log(conf.scriptsdirectory, this.script);
