...
Session.prototype.sync = function() {
  ...
  this.socket.emit('sync', this.layoutState, this.termHistory);
};
...
Session.prototype.handleCreate = function(cols, rows, func, script) {
  ...
  term.on('data', function(data) {
    ...
    self.saveDataToHistory(id, data);
  });
...
Session.prototype.handleKill = function(id) {
  ...
  if (this.termHistory[id]) {
    delete this.termHistory[id];
  }
};
...
Session.prototype.handleDisconnect = function(forceDestroy) {
  ...
  function destroy() {
    ...
    while (i--) {
      ...
      if (termHistory[key[i]]) {
        delete termHistory[key[i]];
      }
...
Session.prototype.saveDataToHistory = function (termId, data) {
  if (!this.termHistory[termId]) {
    this.termHistory[termId] = [];
  }

  this.termHistory[termId].push(data);
};
